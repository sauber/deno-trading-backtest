...
Deno.test("Instance", () => {
  assertInstanceOf(new Instrument(new Float16Array([]), 0, ""), Instrument);
});

Deno.test("Start / End", () => {
  const instr = new Instrument(new Float16Array([0, 1, 2]), 5, "");
  assertEquals(instr.start, 7);
  assertEquals(instr.end, 5);
});

Deno.test("Price", () => {
  const instr = new Instrument(new Float16Array([0, 1]), 0, "");
